...
Minicart.prototype = {
    ...
    init: function() {
        ...
        $j(this.selectors.miniplusminus)
            .unbind('click.quantity')
            .bind('click.quantity', function() {
                if(cart.updateMiniQty(this))
                {
                    cart.processUpdateQuantity(this);
                }
        });
        ...
        if (confirm(el.data('confirm'))) {
            ...
            $j.ajax({
                ...
                if (result.success) {
                    ...
                    if(result.qty <= 0) {
                        $j('a.skip-cart').addClass('no-count');
                    }
                } else {
    ...
    updateMiniQty: function(el) {
        var selclass = jQuery(el).attr('class');
        if(selclass == 'qut-plus miniplusminus')
        { 
            var oldVal = jQuery(el).prev("input").val();
            if ( parseFloat(oldVal) >= 1 ) {
                var newVal = parseFloat(oldVal) + 1;
                jQuery(el).prev("input").val(newVal);
                return true;
            }
            
        } 
        if(selclass == 'qut-minus miniplusminus') 
        {
            var oldVal = jQuery(el).parent().find("input").val();
            if ( parseFloat(oldVal) >= 2 ) {
                var newVal = parseFloat(oldVal) - 1;
                jQuery(el).parent().find("input").val(newVal);
                return true;
            } else {
                return false;
            }
