...
class Multidots_BoxProducts_CartController extends Mage_Checkout_CartController {
    ...
    public function addAction() {
        ...
        try {
            ...
            if ($product->getTypeId() == 'bundle') {
                $cartQty = Mage::helper('boxproducts')->getCartQuantity($product);
                if ($product->getLimitedQty() > $cartQty) {
                    ...
                    $massage = $this->__('%s product have a limited Quantity for purchase.', Mage::helper('core')->escapeHtml($product->getName()));
                    Mage::getSingleton('core/session')->addNotice($massage);
    ...
    public function ajaxUpdateAction() {
        ...
        if ($id) {
            $cart = $this->_getCart();
            $quoteItem = $cart->getQuote()->getItemById($id);
            $products = Mage::getModel('catalog/product')->load($quoteItem->getProductId());
            if ($products->getTypeId() == 'bundle') {
                try {
                    ...
                    if ($qty == 0) {
                        $cart->removeItem($id);
                    } elseif ($products->getLimitedQty() >= $qty) {
                    ...
                    $result['qty'] = $this->_getCart()->getSummaryQty();
                    if ($qty > $products->getLimitedQty()) {
                        $result['notice'] = $this->__('%s product have a limited Quantity for purchase.', $products->getName());
                    } else {
