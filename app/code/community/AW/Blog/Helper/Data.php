...
class AW_Blog_Helper_Data extends Mage_Core_Helper_Abstract {
     ...
     * product Ids are in array formate which will passed from blog page
     */

    public function getProductsCollection($productIds) {
        $_productConditon = array();
        $_prodIds = explode(',', $productIds);
        foreach ($_prodIds as $proId) {
            $_productConditon[] = trim($proId);
        }

        $attributes = Mage::getSingleton('catalog/config')->getProductAttributes();
        $_product_collection = Mage::getModel('catalog/product')
                ->getCollection()
                ->addAttributeToFilter('sku', array('in' => $_productConditon))
                ->addAttributeToSelect($attributes);

        return $_product_collection;
    }

    public function getviewMore($shore_desc,$posturl) {
        // strip tags to avoid breaking any html
        $_shore_desc = strip_tags($shore_desc);

        if (strlen($_shore_desc) > 100) {

            // truncate string
            $stringCut = substr($_shore_desc, 0, 100);

            // make sure it ends in a word so assassinate doesn't become ass...
            $_shore_desc = substr($stringCut, 0, strrpos($stringCut, ' ')) . '... <a href="'.$posturl.'">Read More</a>';
        }
        echo $_shore_desc;
    }
