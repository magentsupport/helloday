...
class Customweb_BarclaycardCw_CheckoutController extends Customweb_BarclaycardCw_Controller_Action
{
	public function breakoutAction()
	{
		...
		if ($transaction->getTransactionObject()->isAuthorizationFailed()) {
			...
			$redirectionUrl = Mage::getUrl('BarclaycardCw/process/success', array('transaction_id' => $transaction->getTransactionId(), '_secure' => true));
		}
		...
		'<html>
			<body>
				...
				' . Mage::helper('BarclaycardCw')->__("You will be redirect to the order confirmation page.") . '
			</body>
		<html>';
				...
				if(!$transaction->getTransactionObject()->isAuthorized()) {
					...
					if ($transaction->getAuthorizationType() == Customweb_Payment_Authorization_Iframe_IAdapter::AUTHORIZATION_METHOD_NAME) {
						$this->getLayout()->getBlock('content')->append(
							$this->getLayout()->createBlock('barclaycardcw/iframe')
						);
					} elseif ($transaction->getAuthorizationType() == Customweb_Payment_Authorization_Widget_IAdapter::AUTHORIZATION_METHOD_NAME) {
						$this->getLayout()->getBlock('content')->append(
							$this->getLayout()->createBlock('barclaycardcw/widget')
						);
					}
					if($transaction->getTransactionObject()->isAuthorizationFailed()){
						$this->_redirect('BarclaycardCw/process/fail',array('transaction_id' => $transaction->getTransactionId(), '_secure' => true));
					}
					else {
						$this->renderLayout();
					}
	...
	public function templateAction()
	{
		...
		$this->getLayout()->getBlock('content')->append(
				$this->getLayout()->createBlock('barclaycardcw/template')
		);
