...
class Customweb_BarclaycardCw_ExternalcheckoutController extends Mage_Core_Controller_Front_Action
{
	...
	public function loginAction()
	{
		...
		$this->getLayout()->getBlock('content')->append(
			$this->getLayout()->createBlock('barclaycardcw/externalCheckout_login')->setContext($context)
		);
		$this->renderLayout();
	}
	...
	public function registerPostAction()
	{
		...
		if ($this->getRequest()->isPost()) {
			...
			if ($dateOfBirth != null) {
				$data['dob'] = Mage::helper('BarclaycardCw/externalCheckout')->getDateAsString($dateOfBirth);
			}
			...
			if (true !== ($result = Mage::helper('BarclaycardCw/externalCheckout')->validateCustomerData($quote, $data, $registerMethod))) {
				if (!is_array($result)) {
					$result = array($result);
				}
		...
		if ($this->getRequest()->isPost()) {
			...
			if (!empty($login['username']) && !empty($login['password'])) {
				try {
					...
					// Mage::helper('BarclaycardCw')->logException($e); // PA DSS violation: this exception log can disclose customer password
				}
			} else {
	...
	/**
	 * @return Customweb_BarclaycardCw_Model_ExternalCheckoutContext
	 */
	private function getContext()
	{
		if ($this->_context == null) {
			$this->_context = Mage::getModel('barclaycardcw/externalCheckoutContext')->loadByQuote($this->getQuote());
		}
