...
class Customweb_BarclaycardCw_Helper_Data extends Mage_Core_Helper_Abstract
{
	private static $container = null;

	public function log($message, $level = null, $file = '', $forceLog = false) {
		if (Mage::getStoreConfig('barclaycardcw/general/debug_log') == '1') {
	...
	public function getAssetResolver() {
		return new Customweb_Asset_Resolver_Composite(array(
			Mage::getModel('barclaycardcw/asset_skinResolver'),
			Mage::getModel('barclaycardcw/asset_jsResolver'),
			Mage::getModel('barclaycardcw/asset_templateResolver'),
			new Customweb_Asset_Resolver_Simple(Mage::getBaseDir('media') . '/customweb/barclaycardcw/assets/', Mage::getBaseUrl('media') . '/customweb/barclaycardcw/assets/')
		));
	}
	...
	{
		$transaction = Mage::getModel('barclaycardcw/transaction')->load($orderPaymentId, 'order_payment_id');
	...
	{
		$transaction = Mage::getModel('barclaycardcw/transaction')->load($orderId, 'order_id');
		if ($transaction !== null && $transaction->getId()) {
			return $transaction;
		}
		...
		//throw new Exception('The transaction could not have been loaded.');
		Mage::getSingleton('adminhtml/session')->addError($this->__('Barclaycard Transaction Missing. The transaction could not have been loaded for OrderID %s', $order->getIncrementId()));
	}

	public function loadTransaction($transactionId)
	{
		$transaction = Mage::getModel('barclaycardcw/transaction')->load($transactionId);
		if ($transaction === null || !$transaction->getId()) {
			return null;
		}
		return $transaction;
	...
	{
		$transaction = Mage::getModel('barclaycardcw/transaction')->load($transactionId, 'transaction_external_id');
		if ($transaction === null || !$transaction->getId()) {
			return null;
		}
		return $transaction;
	...
	{
		$transaction = Mage::getModel('barclaycardcw/transaction')->load($paymentId, 'payment_id');
		if ($transaction === null || !$transaction->getId()) {
			return null;
		}
		return $transaction;
	...
	{
		if ($customer->getData('barclaycardcw_migrated')) {
			return;
		}
		...
		$payments = Mage::getModel('sales/order_payment')->getCollection()
			...
			->addFieldToFilter('method', array('like' => 'barclaycardcw_%'));
		...
		$query = 'UPDATE ' . Mage::getSingleton('core/resource')->getTableName('customer_entity') . ' SET barclaycardcw_migrated = 1 WHERE entity_id = '
				. (int)$customer->getId();
	...
	protected function migrateTransaction(Mage_Sales_Model_Order $order)
	{
		if ($order !== null && $order->getPayment() !== false) {
			...
			if (!empty($additionalData)) {
				...
				$alias = Mage::getModel('barclaycardcw/aliasdata')->load($order->getId(), 'order_id');
	...
	public function getPaymentCustomerContext($customerId = null)
	{
		...
		return Customweb_BarclaycardCw_Model_PaymentCustomerContext::getByCustomerId($id);
	}
	...
	{
		$configAdapter = new Customweb_BarclaycardCw_Model_ConfigurationAdapter();
		return $configAdapter->getConfigurationValue($key);
	}

	public function setConfigurationStoreId($storeId)
	{
		Customweb_BarclaycardCw_Model_ConfigurationAdapter::setStoreId($storeId);
	}
	...
	{
		if (Mage::getStoreConfig('barclaycardcw/general/wait_for_success') != '1') {
