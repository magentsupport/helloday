 ...
 * 
 */
class Customweb_BarclaycardCw_Block_Checkout extends Mage_Core_Block_Template {

	protected function _construct(){
		parent::_construct();
		$this->setTemplate('customweb/barclaycardcw/checkout.phtml');
	}
	...
	public function getPaymentMethods(){
		...
		foreach ($payments as $paymentCode => $paymentModel) {
			if (preg_match("/barclaycardcw/i", $paymentCode)) {
	...
	public function getAuthorizeUrl(){
		return Mage::getUrl('BarclaycardCw/process/authorize', array(
			'_secure' => true 
		));
	}
	...
	public function getSaveShippingMethodUrl(){
		return Mage::getUrl('BarclaycardCw/onepage/saveShippingMethod', array(
			'_secure' => true 
		));
	}
	...
	public function getPreloadUrl(){
		if (version_compare(Mage::getVersion(), '1.8', '>=')) {
			return Mage::getUrl('BarclaycardCw/process/preloadOnepage', array(
				'_secure' => true 
			));
		}
		else {
