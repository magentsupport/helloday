 ...
 *
 */
class Customweb_BarclaycardCw_Block_Form extends Mage_Payment_Block_Form {
	private $printJavascript = true;

	protected function _construct(){
		parent::_construct();
		$this->setTemplate('customweb/barclaycardcw/form.phtml');
	}
	
	public function getContent(){
		
		$arguments = null;
		return Customweb_Licensing_BarclaycardCw_License::run('m91e8pp569uges03', $this, $arguments);
	}

	final public function call_3n1ak5i97jgpb8f9() {
		...
		$form = '
			<div id="payment_description_' . $code . '" class="cw_payment_description">
				' . ($method->getPaymentMethodConfigurationValue('show_image') ? '
					<img src="' . $this->getSkinUrl('images/barclaycardcw/' . $method->getPaymentMethodName() . '.png') . '" /><br/>
	...
	public function getProcessUrl(){
		return Mage::getUrl('BarclaycardCw/process/process', array(
			'_secure' => true 
		));
	}

	public function getJavascriptUrl(){
		return Mage::getUrl('BarclaycardCw/process/ajax', array(
			'_secure' => true 
		));
	}

	public function getHiddenFieldsUrl(){
		return Mage::getUrl('BarclaycardCw/process/getHiddenFields', array(
			'_secure' => true 
		));
	}

	public function getVisibleFieldsUrl(){
		return Mage::getUrl('BarclaycardCw/process/getVisibleFields', array(
			'_secure' => true 
		));
	}
	...
	public function getAliasSelect(){
		...
		if ($payment->getPaymentMethodConfigurationValue('alias_manager') == 'active') {
			...
			if (count($aliasList)) {
				$alias = array(
					'new' => Mage::helper('BarclaycardCw')->__('New card') 
				);
				...
				// on select fields.         	 				  				 	
				$selectControl = new Customweb_BarclaycardCw_Model_Select("alias_select", $alias, 'new', 
						"cwpm_" . $payment->getCode() . ".loadAliasData(this)");
				$aliasElement = new Customweb_Form_Element(Mage::helper('BarclaycardCw')->__("Saved cards:"), $selectControl, 
						Mage::helper('BarclaycardCw')->__("You may choose one of the cards you paid before on this site."));
