...
class Customweb_BarclaycardCw_Model_PaymentCustomerContext implements Customweb_Payment_Authorization_IPaymentCustomerContext
{
	...
	{
		$customerContext = Mage::getModel('barclaycardcw/customercontext')->load($this->customerId);
		if ($customerContext->getId() && $customerContext->getContext()) {
			$result = Mage::helper('BarclaycardCw')->unserialize($customerContext->getContext());
	...
	protected function updateContextMapInDatabase(array $map)
	{
		...
		if (!$customerContext->getId()) {
			$customerContext = Mage::getModel('barclaycardcw/customercontext');
			$customerContext->setId($this->customerId);
		}

		$customerContext->setContext(Mage::helper('BarclaycardCw')->serialize($map));
