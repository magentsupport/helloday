...
class Customweb_BarclaycardCw_Model_MotoObserver
{
	/**
	...
	public function postOrderSave(Varien_Event_Observer $observer)
	{
		...
		if ($orderId) {
			...
			if (strpos($payment->getMethodInstance()
				->getCode(), 'barclaycardcw') !== false) {
				Mage::helper('BarclaycardCw')->log("Codes match.");
				if ($messageType == 'success') {
					$order->addStatusToHistory(Customweb_BarclaycardCw_Model_Method::BARCLAYCARDCW_STATUS_PENDING, Mage::helper('BarclaycardCw')->__('Payment is pending at BarclaycardCw'));
					$order->save();
					Mage::helper('BarclaycardCw')->log("Redirect to Moto authorization");
					...
					$controller->getResponse()
						->setRedirect(Mage::helper('adminhtml')->getUrl('*/motobarclaycardcw/process', array(
	...
	public function beforeOrderSave(Varien_Event_Observer $observer)
	{
		try {
			...
			if (isset($_POST['payment']['method'])) {
				...
				if (strpos($paymentCode, 'barclaycardcw') !== false) {
