 ...
 * @Bean
 */
class Customweb_BarclaycardCw_Model_BackendOperation_RefundAdapter implements Customweb_Payment_BackendOperation_Adapter_Shop_IRefund
{
	public function refund(Customweb_Payment_Authorization_ITransaction $transaction)
	{
		...
		Mage::register('barclaycardcw_update_transaction', $transactionModel);
	...
	protected function createCreditmemo(Mage_Sales_Model_Order $order, $data)
	{
		...
		if ($creditmemo) {
			if (($creditmemo->getGrandTotal() <=0) && (!$creditmemo->getAllowZeroGrandTotal())) {
				Mage::throwException(
					Mage::helper('BarclaycardCw')->__("Credit memo's total must be positive.")
				);
			}

			Mage::register('cw_barclaycardcw_refund_update', true);
