...
class Customweb_BarclaycardCw_Model_TransactionContext implements Customweb_Payment_Authorization_ITransactionContext, Customweb_Payment_Authorization_PaymentPage_ITransactionContext, Customweb_Payment_Authorization_Hidden_ITransactionContext, Customweb_Payment_Authorization_Iframe_ITransactionContext, Customweb_Payment_Authorization_Server_ITransactionContext, Customweb_Payment_Authorization_Moto_ITransactionContext, Customweb_Payment_Authorization_Ajax_ITransactionContext, Customweb_Payment_Authorization_Widget_ITransactionContext, Customweb_Payment_Authorization_IUpdateTransactionContext
{
	...
	public function __construct($orderContext, $orderId, $transactionId, $customerContext, $aliasTransactionId, $backendSuccessUrl, $backendFailUrl, $storeId = null)
	{
		...
		$this->successUrl = $this->appendNonTrackingParameter(Mage::getUrl('BarclaycardCw/process/success', $this->getUrlParameters()));
		$this->failedUrl = $this->appendNonTrackingParameter(Mage::getUrl('BarclaycardCw/process/fail', $this->getUrlParameters()));
	}
	...
	{
		return Mage::helper('BarclaycardCw')->loadTransaction($this->transactionId);
	}
	...
	public function getAlias()
	{
		...
		if ($this->aliasTransactionObject === null) {
			$transaction = Mage::helper('BarclaycardCw')->loadTransaction($this->aliasTransactionId);
	...
	{
		return Mage::getUrl('BarclaycardCw/process/update', array(
			'_secure' => true,
	...
	{
		return Mage::getUrl('BarclaycardCw/process/motoSuccess', array(
			'_secure' => true,
			'_store' => $this->getOrderContext()->getStore()->getId()
		));
	}
	...
	{
		return Mage::getUrl('BarclaycardCw/process/motoFail', array(
			'_secure' => true,
			'_store' => $this->getOrderContext()->getStore()->getId()
		));
	}
	...
	private function getProcessUrl()
	{
		if ($this->processUrl == null) {
			$this->processUrl = $this->appendNonTrackingParameter(Mage::getUrl('BarclaycardCw/process/process', $this->getUrlParameters()));
		}
	...
	{
		return Mage::getUrl('BarclaycardCw/checkout/breakout', $this->getUrlParameters());
	}
