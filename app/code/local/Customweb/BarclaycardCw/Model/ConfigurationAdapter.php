 ...
 * @Bean
 */
class Customweb_BarclaycardCw_Model_ConfigurationAdapter implements Customweb_Payment_IConfigurationAdapter
{
	...
	public function getConfigurationValue($key, $language = null)
	{
		...
		$rs = Mage::getStoreConfig('barclaycardcw/general/' . $key, self::$storeId);
		...
		if (isset($multiSelectKeys[$key])) {
			...
			if (empty($rs) || $defaultValue == $rs) {
				...
				return new Customweb_Core_Stream_Input_File(Mage::getBaseDir('media') . '/barclaycardcw/setting/default/' . $key . '/' . $rs);
			}
		} else {
			return $rs;
		}
	...
	{
		return null != Mage::getStoreConfig('barclaycardcw/general/' . $key, self::$storeId);
	}

	public function getDefaultTemplateUrl()
	{
		$url = Mage::getUrl('BarclaycardCw/checkout/template', array(
			'_secure' => true,
			'_store' => self::$storeId,
			'_store_to_url' => true
		));
