...
class Customweb_BarclaycardCw_Model_Observer
{
	private $timeout = 0;

	public function initCart(Varien_Event_Observer $observer)
	{
		if (Mage::getStoreConfig('barclaycardcw/general/cancel_existing_orders')) {
			...
			$orders = Mage::getResourceModel('sales/order_collection')
				...
				->addAttributeToFilter('status', Customweb_BarclaycardCw_Model_Method::BARCLAYCARDCW_STATUS_PENDING)
				->load();
				...
				foreach ($productIds as $productId) {
					...
					if (!$product->isSalable()) {
						foreach ($orders as $order) {
							...
							$order->setIsActive(0);
							$order->addStatusToHistory(Customweb_BarclaycardCw_Model_Method::BARCLAYCARDCW_STATUS_CANCELED, Mage::helper('BarclaycardCw')->__('Order cancelled, because the ordered products are not available.'));
							$order->save();
						}
						break;
					}
	...
	public function placeOrder(Varien_Event_Observer $observer)
	{
		...
		try {
			if (strpos($order->getPayment()->getMethodInstance()->getCode(), 'barclaycardcw') === 0) {
		...
		try {
			if (strpos($order->getPayment()->getMethodInstance()->getCode(), 'barclaycardcw') === 0) {
				...
				if (Mage::registry('cw_is_moto') == null && !empty($sessionTransactionId)) {
					$transaction = Mage::helper('BarclaycardCw')->loadTransaction($sessionTransactionId);
	...
	public function cancelOrder(Varien_Event_Observer $observer)
	{
		$order = $observer->getOrder();
		if (strpos($order->getPayment()->getMethodInstance()->getCode(), 'barclaycardcw') === 0) {
			$order->addStatusHistoryComment(Mage::helper('BarclaycardCw')->__('Transaction cancelled successfully'));
		}
	}
	...
	public function invoiceView(Varien_Event_Observer $observer)
	{
		...
		if (strpos($invoice->getOrder()->getPayment()->getMethodInstance()->getCode(), 'barclaycardcw') === 0) {
			$transaction = Mage::helper('BarclaycardCw')->loadTransactionByOrder($invoice->getOrder()->getId());
			...
			if (Mage::getSingleton('admin/session')->isAllowed('sales/order/actions/edit')
				...
				$block->addButton('edit', array(
					...
					'class'     => 'go',
					'onclick'   => 'setLocation(\''.$block->getUrl('*/editbarclaycardcw/index', array('invoice_id'=>$invoice->getId())).'\')'
				));
			}
		}
	}
	...
	{
		if (Mage::registry('barclaycardcw_external_checkout_login') === true) {
	...
	public function collectExternalCheckoutWidgets(Varien_Event_Observer $observer)
	{
		...
		$widgets = array_merge($widgets, Mage::getModel('barclaycardcw/externalCheckoutWidgets')->getWidgets());
		$observer->getEvent()->setWidgets($widgets);
	}

	public function registerTranslationResolver(Varien_Event_Observer $observer)
	{
		if (!Mage::registry('customweb_barclaycardcw_transaction_resolver_registered')) {
			Customweb_I18n_Translation::getInstance()->addResolver(new Customweb_BarclaycardCw_Model_TranslationResolver());
			Mage::register('customweb_barclaycardcw_transaction_resolver_registered', true);
		}
	}
	...
	public function migrateCustomerTransactions(Varien_Event_Observer $observer)
	{
		...
		Mage::helper('BarclaycardCw')->migrateCustomersTransactions($customer);
	}
}
