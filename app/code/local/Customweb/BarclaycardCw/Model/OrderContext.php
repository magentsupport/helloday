...
class Customweb_BarclaycardCw_Model_OrderContext extends Customweb_Payment_Authorization_OrderContext_AbstractDeprecated implements Customweb_Payment_Authorization_IOrderContext
{
	/**
	 * Transient fields
	 */
	...
	/**
	 *
	 * @param Customweb_BarclaycardCw_Model_Method $paymentMethod
	 * @param unknown $isOrderAvailable
	 */
	public function __construct(Customweb_BarclaycardCw_Model_Method $paymentMethod, $isOrderAvailable, $storeId = null, $quote = null)
	{
		...
		$checkoutId = $session->getBarclaycardCwCheckoutId();
		if($checkoutId === null) {
			$checkoutId = Customweb_Core_Util_Rand::getUuid();
			$session->setBarclaycardCwCheckoutId($checkoutId);
		}
	...
	{
		$orderContext = new Customweb_BarclaycardCw_Model_OrderContext($order->getPayment()
	...
	{
		return Mage::getModel('barclaycardcw/invoiceItems')->getInvoiceItems($this->getOrderQuote(), $this->getStore()->getStoreId(), $this->useBaseCurrency());
	}
