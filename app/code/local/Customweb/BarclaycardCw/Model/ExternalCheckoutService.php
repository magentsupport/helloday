 ...
 * @Bean
 */
class Customweb_BarclaycardCw_Model_ExternalCheckoutService implements Customweb_Payment_ExternalCheckout_ICheckoutService {
	...
	public function loadContext($contextId, $cache = true){
		return Mage::getModel('barclaycardcw/externalCheckoutContext')->load($contextId);
	}

	public function createSecurityToken(Customweb_Payment_ExternalCheckout_IContext $context){
		if (!($context instanceof Customweb_BarclaycardCw_Model_ExternalCheckoutContext)) {
			throw new Customweb_Core_Exception_CastException('Customweb_BarclaycardCw_Model_ExternalCheckoutContext');
		}
	...
	public function authenticate(Customweb_Payment_ExternalCheckout_IContext $context, $emailAddress, $successUrl){
		...
		if ($quote->isAllowedGuestCheckout() && Mage::getStoreConfig('barclaycardcw/general/external_checkout_account_creation') == 'skip_selection' && !empty($emailAddress)) {
			$customerData = array(
				'email' => $emailAddress,
			...
			if ($dateOfBirth != null) {
				$customerData['dob'] = Mage::helper('BarclaycardCw/externalCheckout')->getDateAsString($dateOfBirth);
			}
			...
			$result = Mage::helper('BarclaycardCw/externalCheckout')->validateCustomerData($quote, $customerData, 'guest');
			if ($result !== true) {
		...
		return Customweb_Core_Http_Response::redirect(Mage::getUrl('BarclaycardCw/Externalcheckout/login', array(
			'_secure' => true
		)));
	}
	...
	public function renderShippingMethodSelectionPane(Customweb_Payment_ExternalCheckout_IContext $context, $errorMessages){
		...
		$block = Mage::getSingleton('core/layout')->createBlock('barclaycardcw/externalCheckout_shippingMethods')->setContext($context);
		return $block->toHtml();
	}
	...
	public function getPossiblePaymentMethods(Customweb_Payment_ExternalCheckout_IContext $context){
		...
		foreach ($activePaymentMethods as $paymentMethodCode => $paymentMethod) {
			if (strpos($paymentMethodCode, 'barclaycardcw') === 0) {
	...
	public function renderReviewPane(Customweb_Payment_ExternalCheckout_IContext $context, $renderConfirmationFormElements, $errorMessage){
		...
		$update->load('barclaycardcw_external_checkout_review');
		$layout->generateXml();
	...
	public function validateReviewForm(Customweb_Payment_ExternalCheckout_IContext $context, Customweb_Core_Http_IRequest $request){
		...
		$helper = Mage::helper('BarclaycardCw');

		$quote = $context->getQuote();
		if ($context->hasBasketChanged($quote, $request)) {
			$context->updateFromQuote($quote);
			$context->save();
			throw new Exception(Mage::helper('BarclaycardCw')->__('The shopping cart has been altered!'));
		}
	...
	public function renderAdditionalFormElements(Customweb_Payment_ExternalCheckout_IContext $context, $errorMessage){
		...
		$block = Mage::getSingleton('core/layout')->createBlock('barclaycardcw/externalCheckout_additionalInformation')->setContext($context);
		return $block->toHtml();
	}
	...
	public function processAdditionalFormElements(Customweb_Payment_ExternalCheckout_IContext $context, Customweb_Core_Http_IRequest $request){
		...
		if (Mage::helper('BarclaycardCw/externalCheckout')->isGenderRequired($context->getQuote())) {
		...
		if (Mage::helper('BarclaycardCw/externalCheckout')->isDateOfBirthRequired($context->getQuote())) {
	...
	public function createOrder(Customweb_Payment_ExternalCheckout_IContext $context){
		...
		try {
			...
			Mage::helper('BarclaycardCw')->logException($e);
			throw $e;
		}
	}
	...
	private function prepareGuestQuote(Customweb_BarclaycardCw_Model_ExternalCheckoutContext $context){
		$quote = $context->getQuote();
	...
	private function prepareNewCustomerQuote(Customweb_BarclaycardCw_Model_ExternalCheckoutContext $context){
		$quote = $context->getQuote();
	...
	private function prepareCustomerQuote(Customweb_BarclaycardCw_Model_ExternalCheckoutContext $context){
		$quote = $context->getQuote();
		$billing = $quote->getBillingAddress();
		$shipping = $quote->isVirtual() ? null : $quote->getShippingAddress();
	...
	private function involveNewCustomer(Customweb_BarclaycardCw_Model_ExternalCheckoutContext $context){
		$customer = $context->getQuote()->getCustomer();
		Mage::getSingleton('customer/session')->loginById($customer->getId());
	}

	private function refreshContext(Customweb_BarclaycardCw_Model_ExternalCheckoutContext $context){
		if (!($context instanceof Customweb_BarclaycardCw_Model_ExternalCheckoutContext)) {
			throw new Customweb_Core_Exception_CastException('Customweb_BarclaycardCw_Model_ExternalCheckoutContext');
		}
...
// 			}
		...
		if (!empty($paymentMethod)) {
			...
			$data = array(
				'method' => 'barclaycardcw_' . $paymentMethod,
				'checks' => 4 | 1 | 2 | 32 | 128,
			);

			$quote->setBarclaycardCwExternalCheckout(true);
		...
		if ($quote->isVirtual()) {
			$context->setShippingMethodName(Mage::helper('BarclaycardCw')->__('No shipping method needed.'));
		} else {
		...
		if ($context->getBillingAddress() !== null && $context->getBillingAddress() instanceof Customweb_Payment_Authorization_OrderContext_IAddress) {
			if (!in_array($context->getBillingAddress()->getCountryIsoCode(), $allowCountries)) {
				throw new Exception(Mage::helper('BarclaycardCw')->__('It is not possible to checkout in your country.'));
			}
		}
	...
	private function updateUserSessionWithCurrentUser(Customweb_BarclaycardCw_Model_ExternalCheckoutContext $context){
		if (!($context instanceof Customweb_BarclaycardCw_Model_ExternalCheckoutContext)) {
			throw new Customweb_Core_Exception_CastException('Customweb_BarclaycardCw_Model_ExternalCheckoutContext');
		}
		...
		$quote = $context->getQuote();
		if ($customer->getId()) {
			Mage::register('barclaycardcw_external_checkout_login', true);
			Mage::getSingleton('customer/session')->setCustomerAsLoggedIn($customer);
			$quote->setCheckoutMethod(Mage_Checkout_Model_Type_Onepage::METHOD_CUSTOMER)->save();
			Mage::unregister('barclaycardcw_external_checkout_login');
		}
