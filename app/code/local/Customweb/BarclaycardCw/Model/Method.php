...
new Customweb_BarclaycardCw_Model_TranslationResolver();

abstract class Customweb_BarclaycardCw_Model_Method extends Mage_Payment_Model_Method_Abstract implements 
		Customweb_Payment_Authorization_IPaymentMethod {
	protected $_code = 'barclaycardcw';
	protected $_formBlockType = 'barclaycardcw/form';
	protected $_infoBlockType = 'barclaycardcw/info';
	
	/**
	 * Order statuses         	 				  				 	
	 */
	const BARCLAYCARDCW_STATUS_PENDING = 'pending_barclaycardcw';
	const BARCLAYCARDCW_STATUS_CANCELED = 'canceled_barclaycardcw';
	...
	protected function setOrderStatus(&$order, $status, $logEntry = null){
		try {
			...
			Mage::dispatchEvent('barclaycardcw_order_status', array(
				'order' => $order,
				'status' => $status 
			));
		}
	...
	public function isAvailable($quote = null){
		...
		if ($isAvailable && $quote !== null && $quote->getBarclaycardCwExternalCheckout() !== true) {
	 ...
	 * @return Customweb_BarclaycardCw_Model_Method
	 */
	public function initialize($paymentAction, $stateObject){
		$stateObject->setStatus(self::BARCLAYCARDCW_STATUS_PENDING);
	...
	public function validate(){
		...
		$arguments = null;
		return Customweb_Licensing_BarclaycardCw_License::run('rs084da9vtr8ihh6', $this, $arguments);
	}

	final public function call_enmu8h3v6fk7b88s() {
	 ...
	 * @return string
	 */
	public function processServerAuthorization(Customweb_BarclaycardCw_Model_Transaction $transaction, array $parameters){
	...
	public function cancel(Varien_Object $payment){
		...
		try {
			if (Mage::registry('barclaycardcw_update_transaction') != null) {
				$transaction = Mage::registry('barclaycardcw_update_transaction');
			}
			else {
				$transaction = $this->getHelper()->loadTransactionByPayment($payment->getId());
			}
			$order = $payment->getOrder();
			Customweb_BarclaycardCw_Model_ConfigurationAdapter::setStore($order);
			...
			if (!$transaction->getTransactionObject()->isCancelled() && $order->getStatus() != self::BARCLAYCARDCW_STATUS_PENDING) {
				$this->getHelper()->log(
	 ...
	 * @param float $amount
	 *
	 * @return Mage_BarclaycardCw_Model_Method
	 */
	public function refund(Varien_Object $payment, $amount){
		...
		try {
			...
			$refunded = false;
			if ($transaction->getTransactionObject()->isRefundPossible() && Mage::registry('cw_barclaycardcw_refund_update') !== true) {
			...
			if ($refunded || Mage::registry('cw_barclaycardcw_refund_update') === true) {
				...
				$payment->getCreditmemo()->setBarclaycardcwRefundId($refund->getRefundId());
	...
	protected function getInvoiceItems($invoice, $amount, $currency, $useBaseCurrency = false){
		...
		$invoiceItems = Mage::getModel('barclaycardcw/invoiceItems');
	 ...
	 */
	public function processPayment(Mage_Sales_Model_Order $order, Customweb_BarclaycardCw_Model_Transaction $transaction){
		if ($order->getId()) {
			Customweb_BarclaycardCw_Model_ConfigurationAdapter::setStore($order);
			Mage::getSingleton('core/session')->unsetBarclaycardCwTransactionId();
			if ($transaction->getTransactionObject()->isAuthorized()) {
				$transaction->saveIgnoreOrderStatus();
				$order->setState(self::STATE_PROCESSING);
				$order->save();
				
				Mage::dispatchEvent('barclaycardcw_payment_success', array(
					'order' => $order 
				));
				...
				if ($transaction->getTransactionObject()->isAuthorizationUncertain()) {
					...
					$order->addStatusHistoryComment(
							$this->getHelper()->__('Customer successfully returned from Barclaycard') . "\n<br>PaymentId:" .
									 $transaction->getTransactionObject()->getPaymentId());
					$order->save();
					$this->createAuthPaymentTransaction($order, $transaction);
					if ($this->createInvoice($order, $transaction)) {
						...
						Mage::dispatchEvent('barclaycardcw_invoice_created', array(
							'order' => $order 
						));
					}
				}
	 ...
	 *
	 * @param Customweb_BarclaycardCw_Model_Transaction $transaction
	 */
	public function createInvoiceAfterUncertain(Customweb_BarclaycardCw_Model_Transaction $transaction){
	...
	protected function createAuthPaymentTransaction(Mage_Sales_Model_Order $order, Customweb_BarclaycardCw_Model_Transaction $transaction){
	 ...
	 */
	public function createInvoice(Mage_Sales_Model_Order $order, Customweb_BarclaycardCw_Model_Transaction $transaction){
		$result = false;
	...
	public function getOrderPlaceRedirectUrl(){
		...
		if ($adapter instanceof Customweb_Payment_Authorization_PaymentPage_IAdapter) {
			return Mage::getUrl('BarclaycardCw/process/ppRedirect', 
					array(
		...
		elseif ($adapter instanceof Customweb_Payment_Authorization_Iframe_IAdapter ||
			...
			return Mage::getUrl('BarclaycardCw/checkout/pay', 
					array(
						'transaction_id' => $transactionId,
						'secret' => $secretHash,
						'_secure' => true 
			...
			return Mage::getUrl('BarclaycardCw/process/dummy', array(
				'_secure' => true 
			));
		}
	}

	public function generateHiddenFormParameters(Customweb_BarclaycardCw_Model_Transaction $transaction){
		$adapter = $this->getAuthorizationAdapter(true, $transaction);
	...
	public function getFormRenderer(){
		$renderer = new Customweb_BarclaycardCw_Model_FormRenderer();
		$renderer->setNameSpacePrefix($this->getCode());
		return $renderer;
	}

	public function generateJavascriptForAjax(Customweb_BarclaycardCw_Model_Transaction $transaction){
		$adapter = $this->getAuthorizationAdapter(true, $transaction);
	...
	public function fail(Customweb_BarclaycardCw_Model_Transaction $transaction, array $parameters){
		$order = $transaction->getOrder();
		
		Mage::dispatchEvent('barclaycardcw_payment_fail', array(
			'order' => $order 
		));
		...
		if ($order->getState() != Mage_Sales_Model_Order::STATE_CANCELED &&
			...
			//$order->setIsActive(0);
			$this->setOrderStatus($order, Customweb_BarclaycardCw_Model_Method::BARCLAYCARDCW_STATUS_CANCELED, 
	...
	public function success(Customweb_BarclaycardCw_Model_Transaction $transaction, array $parameters){
		$quote = Mage::getModel('sales/quote');
	...
	public function pullUpdate(Customweb_BarclaycardCw_Model_Transaction $transaction){
	...
	public function redirectToPaymentPage(Customweb_BarclaycardCw_Model_Transaction $transaction, array $parameters){
		$info = $this->getInfoInstance();
	...
	public function getFormHtml($adapter, Customweb_BarclaycardCw_Model_Transaction $transaction, $javaScript = true, $additionalFields = "", $formData = null){
		if ($formData === null) {
			$formData = $_REQUEST;
		}
	...
	public function startMotoAuthorization(Mage_Sales_Model_Order $order, array $parameters){
		...
		$orderContext = Customweb_BarclaycardCw_Model_OrderContext::fromOrder($order);
	...
	public function generateMotoForm(Mage_Sales_Model_Order $order, Customweb_BarclaycardCw_Model_Transaction $transaction, $parameters){
		$adapter = $this->getHelper()->getAuthorizationAdapter(Customweb_Payment_Authorization_Moto_IAdapter::AUTHORIZATION_METHOD_NAME);
		$failedTransaction = $this->loadFailedTransaction();
		$paymentCustomerContext = $this->getHelper()->getPaymentCustomerContext($order->getCustomerId());
		...
		$renderer = new Customweb_BarclaycardCw_Model_BackendFormRenderer();
	...
	public function getIFrameUrl(Customweb_BarclaycardCw_Model_Transaction $transaction, array $parameters){
		$adapter = $this->getAuthorizationAdapter(true, $transaction);
		$url = "";
	...
	public function getIFrameHeight(Customweb_BarclaycardCw_Model_Transaction $transaction, array $parameters){
		$adapter = $this->getAuthorizationAdapter(true, $transaction);
		$height = 1000;
	...
	public function getWidgetHtml(Customweb_BarclaycardCw_Model_Transaction $transaction, array $parameters){
		$adapter = $this->getAuthorizationAdapter(true, $transaction);
		$url = "";
	...
	public function getPaymentMethodConfigurationValue($key, $languageCode = null){
		$result = null;
		$rs = Mage::getStoreConfig('payment/' . $this->getCode() . '/' . $key, Customweb_BarclaycardCw_Model_ConfigurationAdapter::getStoreId());
		...
		elseif (isset($fileKeys[$key])) {
			...
			if (empty($rs) || $defaultValue == $rs) {
				return Mage::helper('BarclaycardCw')->getAssetResolver()->resolveAssetStream($defaultValue);
			}
			else {
				return new Customweb_Core_Stream_Input_File(
						Mage::getBaseDir('media') . '/barclaycardcw/setting/' . $this->getPaymentMethodName() . '/' . $key . '/' . $rs);
			}
		}
		else {
			$result = $rs;
	...
	public function existsPaymentMethodConfigurationValue($key, $languageCode = null){
		...
		return null != Mage::getStoreConfig('payment/' . $this->getCode() . '/' . $key, 
				Customweb_BarclaycardCw_Model_ConfigurationAdapter::getStoreId());
	}
	 ...
	 * @return NULL
	 */
	public function getAuthorizationAdapter($isOrderAvailable, Customweb_BarclaycardCw_Model_Transaction $transaction = null){
	...
	public function getOrderContext($isOrderAvailable, $quote = null){
		if ($this->orderContext == null) {
			$orderContext = new Customweb_BarclaycardCw_Model_OrderContext($this, $isOrderAvailable, null, $quote);
	 ...
	 *
	 * @param Mage_Sales_Model_Order $order
	 * @return Customweb_BarclaycardCw_Model_Transaction
	 */
	public function createTransaction(Mage_Sales_Model_Order $order, $moto = false){
		$transaction = Mage::getModel('barclaycardcw/transaction');
		$transaction->save();
		
		Mage::getSingleton('core/session')->setBarclaycardCwTransactionId($transaction->getId());
	 ...
	 * @return Customweb_BarclaycardCw_Model_Transaction
	 */
	public function updateTransaction(Customweb_BarclaycardCw_Model_Transaction $transaction, Mage_Sales_Model_Order $order, $moto = false){
		if (!$transaction->getOrderId() && $order->getId()) {
			...
			$session->unsBarclaycardCwCheckoutId();
	 ...
	 * @return Customweb_BarclaycardCw_Model_TransactionContext
	 */
	public function getTransactionContext(Mage_Sales_Model_Order $order, Customweb_BarclaycardCw_Model_Transaction $transaction, $backendSuccessUrl = '', $backendFailUrl = ''){
		$aliasTransactionId = Mage::getSingleton('checkout/session')->getAliasId();
		$storeId = $order->getStore()->getId();
		$orderContext = Customweb_BarclaycardCw_Model_OrderContext::fromOrder($order);
		$transactionContext = new Customweb_BarclaycardCw_Model_TransactionContext($orderContext, $order->getIncrementId(), 
	 ...
	 * @return Customweb_BarclaycardCw_Model_TransactionContext
	 */
	public function getMotoTransactionContext(Mage_Sales_Model_Order $order, Customweb_BarclaycardCw_Model_Transaction $transaction){
		$storeId = $order->getStore()->getId();
		$orderContext = Customweb_BarclaycardCw_Model_OrderContext::fromOrder($order);
		
		$backendSuccessUrl = Mage::getModel('adminhtml/url')->getUrl('adminhtml/motobarclaycardcw/success', 
				array(
					'order_id' => $order->getId() 
				));
		$backendFailUrl = Mage::getModel('adminhtml/url')->getUrl('adminhtml/motobarclaycardcw/fail', array(
			'order_id' => $order->getId() 
		));
	...
	/**
	 *
	 * @return Customweb_BarclaycardCw_Helper_Data
	 */
	public function getHelper(){
		if ($this->helper == null) {
			$this->helper = Mage::helper('BarclaycardCw');
		}
		return $this->helper;
	}
	 ...
	 */
	protected function getErrorMessageToDisplay(Customweb_BarclaycardCw_Model_Transaction $transaction){
	...
	public function loadAliasForCustomer(){
		...
		if (Mage::helper('customer')->isLoggedIn()) {
			...
			$collection = Mage::getModel('barclaycardcw/transaction')->getCollection()->addFieldToFilter('customer_id', 
					array(
						'eq' => $customerId 
