...
class Mage_Checkout_Model_Cart extends Varien_Object implements Mage_Checkout_Model_Cart_Interface {
    ...
    public function addOrderItem($orderItem, $qtyFlag = null) {
        ...
        if (is_null($orderItem->getParentItem()) && !in_array($orderItem->getProduct()->getSku(), $skip_items)) {
        ...
        foreach ($data as $itemId => $itemInfo) {
            ...
            if ($products->getTypeId() == 'bundle') {
                ...
                if ($products->getLimitedQty() > $qty) {
                    ...
                    $session->addNotice(Mage::helper('checkout')->__('%s product have a limited Quantity for purchase.', $products->getName()));
                }
            } else {
                if (!$item) {
                    continue;
